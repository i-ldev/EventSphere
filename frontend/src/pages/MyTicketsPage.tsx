...
export default function MyTicketsPage() {
  ...
  // 2. Download PDF Functionality (Isolated Print Window)
  const handleDownloadPDF = (reg: Registration, seat: string) => {
    // Find the SVG element we rendered for this specific ticket
    ...
    // Write clean HTML for just the ticket into the new window
  ...
  // 3. Add to Wallet Functionality (Mock)
      ...
      {registrations.length === 0 ? (
        ...
        <div className="grid grid-cols-1 gap-6 md:grid-cols-2">
          {registrations.map((reg) => {
            // Mocking a seat number for the UI
            const mockSeat = `A${Math.floor(Math.random() * 20) + 1}`;

            return (
              <div
                key={reg.id}
                className="overflow-hidden bg-zinc-900 border border-zinc-800 rounded-xl shadow-xl"
              >
                <div className="p-6">
                  ...
                  <div className="flex items-center gap-4 p-4 bg-zinc-950 rounded-lg border border-zinc-800">
                    <div className="p-3 bg-white border border-zinc-200 rounded-lg">
                      {/* Added an ID here so we can grab it for the PDF print */}
                      <QRCodeSVG
                        id={`qr-${reg.id}`}
                        value={reg.id}
                        size={100}
                  ...
                  <div className="grid grid-cols-3 gap-2 mt-4">
                    <button
                      onClick={() => handleDownloadPDF(reg, mockSeat)}
