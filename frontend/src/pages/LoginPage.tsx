  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.data.success) {
        setAuth({ id: '', email: formData.email, role: '' }, response.data.data.accessToken);
      ...
      setError(axiosError.response?.data?.message || 'Login failed. Check console.');
    }
  };
    ...
    <div className="flex items-center justify-center min-h-screen bg-zinc-950">
      <div className="w-full max-w-md p-8 space-y-6 bg-zinc-900 border border-zinc-800 rounded-xl shadow-2xl">
        <div className="flex flex-col items-center justify-center mb-6">
          ...
          <p className="text-sm text-zinc-500">Sign in to your EventSphere workspace</p>
        </div>
