...
import { IRegistrationRepository, OrganizerStats, PopulatedRegistration } from '../../domain/repositories/IRegistrationRepository.js';
import { Registration } from '../../domain/entities/Registration.js';
import { RegistrationModel, IRegistrationDocument } from '../database/models/RegistrationModel.js';
...
export class MongoRegistrationRepository implements IRegistrationRepository {
  ...
  async findByUserId(userId: string): Promise<PopulatedRegistration[]> {
    // Populate the specific fields we need, including the Jitsi meeting URL
    const docs = await RegistrationModel.find({ userId }).populate('eventId', 'title date location isVirtual meetingUrl');
    
    return docs.map(doc => {
      const event = doc.eventId as any;
