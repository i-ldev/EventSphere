...
import { RegisterUserUseCase, RegisterUserDTO } from '../../application/user/RegisterUserUseCase.js';
import { LoginUserUseCase, LoginUserDTO } from '../../application/user/LoginUserUseCase.js';
...
export class AuthController {
  ...
  register = async (req: AuthenticatedRequest, res: Response): Promise<void> => {
    try {
      ...
      res.status(201).json({ success: true, message: 'User registered successfully', data: { id: user.id, email: user.email, role: user.role } });
    } catch (error) {
  ...
  login = async (req: AuthenticatedRequest, res: Response): Promise<void> => {
    try {
      ...
      const { accessToken, refreshToken } = await this.loginUserUseCase.execute(dto);
